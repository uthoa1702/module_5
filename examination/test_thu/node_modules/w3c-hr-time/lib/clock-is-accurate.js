...
function testClockAccuracy() {
  // OrderList hrtime() first. The check is simpler and more stable, and we use hrtime() to measure Date.now()'s performance.
  const roundTrip = hrtime(hrtime());
  if (roundTrip[0] > 1 || roundTrip[1] > 5e3 * 2) {
    return false;
  }

  // OrderList Date.now() twice: first with a looser bound (10 μs) but with a smaller run time to filter out very bad
